...
class owner(db.Model):
    ...
    profileData = db.relationship("Age", backref="user_data", uselist=False)
...
class Profile(db.Model):
    ...
    user_data = db.Column(db.Integer, db.ForeignKey("owner.id"))


class Allergies(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    users_food_allergies = db.Column(db.Integer, db.ForeignKey("owner.id"))


class DietaryRestriction(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    users_food_dietary = db.Column(db.Integer, db.ForeignKey("owner.id"))
